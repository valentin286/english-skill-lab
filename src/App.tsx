...
export default function App() {
  ...
  const generateComic = async () => {
    ...
    try {
      ...
      } else if (selectedTopic.id === 'future-forms') {
         prompt += "Left panel: A person dropping a stack of books, another person stepping in saying 'I will help!' (Instant decision). Right panel: A person holding an airline ticket and a suitcase saying 'I am going to fly' (Plan/Arrangement). 16-bit pixel art.";
      } else if (selectedTopic.id === 'future-perfect') {
         prompt += "Left panel: A student holding a diploma with a big smile, caption 'I will have finished' (Future Perfect Simple). Right panel: A student sitting at a desk with a huge stack of books, looking tired but determined, clock showing late time, caption 'I will have been studying for 12 hours' (Future Perfect Continuous). 16-bit pixel art.";
      } else if (selectedTopic.id === 'mixed-tenses') {
         prompt += "Left panel: A writer holding 3 sealed letters, smiling, caption 'I have written 3 letters' (Result). Right panel: A writer at a messy desk with crumpled papers, looking exhausted, clock showing late night, caption 'I have been writing all day' (Duration). 16-bit pixel art.";
      } else if (selectedTopic.id === 'relative-clauses') {
         prompt += "Left panel: A group of green apples, one is red. Caption: 'The apple that is red is mine' (Defining). Right panel: A single red apple on a table. Caption: 'The apple, which is red, is mine' (Non-defining). 16-bit pixel art.";
      } else {
         prompt += "A funny situation showing the difference between two grammar concepts. 16-bit pixel art.";
      }
  ...
  const startExam = (topic: Topic) => {
    const sourceQuestions = (topic.examQuestions && topic.examQuestions.length > 0) 
      ? topic.examQuestions 
      : topic.manualQuestions;
    ...
    setCurrentView('exam');
    startSessionTracking();
    const shuffled = [...sourceQuestions].sort(() => 0.5 - Math.random()).slice(0, 20);
